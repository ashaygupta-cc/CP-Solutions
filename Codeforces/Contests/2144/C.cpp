...
vector<vector<ll>> dp;
 
ll rec(ll idx,bool IslastSwap,vector<ll>&a,vector<ll>&b,ll n){
    
    if(idx == n) return 1;
    
    if(dp[idx][IslastSwap] != -1) return dp[idx][IslastSwap];
    
    ll count = 0;
    
    if(idx == 0){
        count = (count+rec(idx+1,0,a,b,n)) % mod2;
        count = (count+rec(idx+1,1,a,b,n)) % mod2;
    }
    else{
        if(!IslastSwap){
    ...
    return dp[idx][IslastSwap] = count;
}
 
void ZodiacZ408(){
