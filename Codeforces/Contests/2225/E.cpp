...
void ZodiacZ408(){
   ...
   read(i,0,n){
       ...
       maxiX = max(maxiX,p[i].first);
       miniY = min(miniY,p[i].second);
       maxiY = max(maxiY,p[i].second);
   }
   
   map<pair<ll,ll>,vector<ll>>grids;
   
   read(i,0,n){
       ll pXGrid = floor((double)p[i].first/(2*r));
       ll pYGrid = floor((double)p[i].second/(2*r));
       grids[{pXGrid,pYGrid}].push_back(i);
   }
   
   ll height = ceil(sqrt(3.0)*r); 
   
   vector<pair<ll,ll>>ans;
   
   ll maxRows = (maxiY-miniY)/height+4;
   
   read(row,-2,maxRows){
       
       ll cury = maxiY+2*r-row*height;
       
       bool bl = false;
       if(abs(row)%2) bl = true;
       
       for(ll xi = miniX-2*r; xi <= maxiX+2*r; xi += 2*r){
           
           ll curx = xi+(bl ? r : 0);
           
           ll XGrid = floor((double)curx/(2*r));
           ll YGrid = floor((double)cury/(2*r));
           ...
           read(dx,-1,2){
               read(dy,-1,2){
                   ...
                   auto it = grids.find({XGrid+dx,YGrid+dy});
