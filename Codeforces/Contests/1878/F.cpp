...
// ll fact[MAXN],invfact[MAXN],prime[MAXN];
ll spf[MAXN],pf_cnt[MAXN],pref_pf[MAXN];
// vector<ll>pr;
 
...
map<ll,ll>mp1;
 
void get_pf_cnt(map<ll,ll>&m,ll x){
    while(x>1){
        m[spf[x]]++;
        x/=spf[x];
    }
