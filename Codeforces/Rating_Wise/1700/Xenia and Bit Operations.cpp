...
 
// When Point update then no need of lazy propagation.
// Needed only when (range update & range query) or (range update & point query)
 
class DarkSeggyTree{
  ...
  public: 
    ...
    void buildRelationship(int node,int s,int e,vector<ll>& a,bool isOr){
        ...
        int mid = s+(e-s)/2;
        buildRelationship(2*node,s,mid,a,!isOr);
        buildRelationship(2*node+1,mid+1,e,a,!isOr);
        
        if(isOr) tree[node] = tree[2*node] | tree[2*node+1];
...
 
    ...
    void updateMeBaby(int node,int s,int e,int idx,int val,bool isOr){
        ...
        if(idx <= mid) updateMeBaby(2*node,s,mid,idx,val,!isOr);
        else updateMeBaby(2*node+1,mid+1,e,idx,val,!isOr);
        
        if(isOr) tree[node] = tree[2*node] | tree[2*node+1];
        else tree[node] = tree[2*node]^tree[2*node+1];
    }
    
    void turnMeOn(int idx,int val,bool isOr){
       updateMeBaby(1,0,n-1,idx,val,isOr);
    }
...
void ZodiacZ408(){
   ...
   bool bl = h&1;
   DST.buildRelationship(1,0,n-1,a,bl);
   ...
   read(i,0,m){
      ...
      DST.turnMeOn(x-1,y,bl);
      a[x-1] = y;
      
      cout << DST.getXor() << endl;
   }
 
}
 
