...
 
 ...
 // ll prime[MAXN],spf[MAXN],pf_cnt[MAXN],pref_pf[MAXN];
// vector<ll>pr;
 
...
 
   ...
   read(i,0,n+1){
       ...
       read(j,i+1,r+1){
          ll mex = j;
          ll takenBef = mex-i-1;
          ll takenAfter = i-takenBef;
          
          ll ways = (nCr(mex-1, takenBef)*nCr(n-mex, takenAfter))%mod;
          ans = (ans+(mex*ways)%mod)%mod;
       }
       
       if(2*i+1 > n){
           ll ways = nCr(n,i);
           ans = (ans+((2*i+1)*ways)%mod)%mod;
       }
   }
   cout << ans << endl;
}
