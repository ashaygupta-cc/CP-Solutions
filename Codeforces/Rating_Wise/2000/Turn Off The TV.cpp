...
// Concept+MyApproach: 
 
   // Basically the approach is easy seg tree ka use karke min store karo for l,r considering the overall range ab kisi query l,r me agar vo poori ka min greater than 1 hai matlab saara kisi ke saath must overlapped hai ! To ek hi answer dena hai any of them so this is the answer!  
   // Bas hard part ye tha ki isme  l,r ki range 1e9 hai to array to bana nhi sakte ki uska seg tree bana do ! 
   // Technqiue : Coordinate Compression to yahan par ye technqiue me kya ki l,r,r+1 saare store kar lo unique ab fir iske upar lower bound l and r me poora +1 and next to that -1 basically pref sum ab iske upar use karo seg tree (max size possible is 6*1e5 (considering all l,r distinct and r+1 too!));
//
 
class DarkSeggyTree{
...
 
   read(i,0,n){
      ...
      ll l = lower_bound(c.begin(),c.end(),q[i].first)-c.begin();
      ll r = lower_bound(c.begin(),c.end(),q[i].second)-c.begin();
 
      a[l]++;
      a[r+1]--; 
   }
   ...
   DarkSeggyTree DST(x);
   DST.buildRelationship(1,0,x-1,a);
