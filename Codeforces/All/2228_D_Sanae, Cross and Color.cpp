...
void ZodiacZ408(){
   ...
   vector<ll>prefMax(n),prefMin(n),suffMax(n),suffMin(n);
   
   ll mini = LLONG_MAX;
   ll maxi = LLONG_MIN;
   
   read(i,0,n){
       maxi = max(maxi,c[i].second);
   ...
   read(i,0,n-1){
       ...
       if(c[i].first == c[i+1].first) continue;
       
       ll l = max(prefMin[i],suffMin[i+1]);
       ll r = min(prefMax[i],suffMax[i+1])-1;
       
       if(l <= r){
           ll upperl = upper_bound(Ys.begin(),Ys.end()-1,r)-Ys.begin();
           ll lowerl = lower_bound(Ys.begin(),Ys.end()-1,l)-Ys.begin();
           
           tot += max(0LL,upperl-lowerl);
       }
   }
