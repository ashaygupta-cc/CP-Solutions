...
// Concept+MyApproach: 
 
  // Algorithm Name : Mo's Algorithm : Jab range queries ho updatation na ho rha ho aur han tum seg tree bhi use nhi kar sakte kyunki tumhara tree ke nodes me count store nhi kar paaoge ya sum store nhi kar paaoge ! Basically is question me tumhe karna hai ki jo sum of prod of Ks*Ks*s;
  
  // Ab Ks isme us particular number ka us query me count hai ab tumne maan lo tree bana diya storing the count traversing above but kaise karoge MLE dega if made is like sum,count ! Aur sum of count ke hisaab se change karna again difficult based on query (l,r);
  
  // To iske liye Mo's Algorithm basically ye kya karta hai query ko l,r,idx ke base pe sort kar deta hai and then based on the closest ones ki usme kya hata ke ya kya add karke next query ka result mil jaaye ! Ye efficient hai as it take sqrt(N) per query for all queries it is O((N+Q)*sqrt(N));
...
void ZodiacZ408(){
    ...
    sort(query.begin(),query.end(),[&](const Info &a,const Info &b){
        if(a.l/x != b.l/x) return a.l < b.l;
        if((a.l/x) & 1) return a.r > b.r;
        return a.r < b.r;
    });
