...
 
    ...
    cin >> n >> x;
 
    ll lZeros = (((x-1)/4)+1+(((x-1)%4 == 3) ? 1 : 0)) % mod2;
    ll rZeros = ((n/4)+1+((n%4 == 3) ? 1 : 0)-lZeros+mod2) % mod2;
 
    ll lOnes = ((x-1 > 0) ? (((x-2)/4)+1) : 0) % mod2;
    ll rOnes = ((((n-1)/4)+1)-lOnes+mod2) % mod2;
 
    ll ans = 0;
 
    ans = (ans+(lZeros*rZeros)%mod2)%mod2;
    ans = (ans+(lOnes*rOnes)%mod2)%mod2;
