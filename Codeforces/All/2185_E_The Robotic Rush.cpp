...
void ZodiacZ408(){
   ...
   vector<ll>firstL(k+2,LLONG_MAX);
   vector<ll>firstR(k+2,LLONG_MAX);
   ...
   read(i,0,k){
       ...
       if(count < 0 && abs(count) <= k && firstL[abs(count)] == LLONG_MAX) 
           firstL[abs(count)] = i+1;
       
       if(count > 0 && count <= k && firstR[count] == LLONG_MAX) 
           firstR[count] = i+1;
   }
   ...
   read(i,0,n){
       ...
       ll curMaxi = LLONG_MAX,curmini = LLONG_MAX;
       
       ll it = lower_bound(b.begin(),b.end(),a[i])-b.begin();
       
       if(it < m){
          ll dist = b[it]-a[i];
          if(dist <= k) curMaxi = firstR[dist];
       }
       
       if(it-1 >= 0){
          ll dist = a[i]-b[it-1];
          if(dist <= k) curmini = firstL[dist];
       }
       
       d.push_back(min(curMaxi,curmini));
   }
   ...
   read(i,1,k+1){
       ...
       ll dead = upper_bound(d.begin(),d.end(),i)-d.begin();
 
       cout << n-dead << " ";
   }
   cout << endl;
